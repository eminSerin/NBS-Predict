function [NBSPredict] = run_NBSPredict_Top5(NBSPredict)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% run_NBSPredict_Top5 runs Top 5% feature selection algorithm to perform
...
%   - Features (edges) are selected by selecting top 5% of features based
%       on test statistics.
...
NBSPredict.parameter.percent = 5; % Top 5%
...
[outerFoldCVresults] = crossValidation(modelEvaluateFun,data,...
    ...
    function [modelEvalResults] = modelEvaluate(data,NBSPredict)
        ...
        extIdx = run_TopK(filterData,NBSPredict);
...
function [extIdx] = run_TopK(data,NBSPredict)
% run_TopK is a univariate feature selection method that selects top K% of
% features based on test statistics.
...
% Select n features.
extIdx = I(1:round(numel(I)/(100/NBSPredict.parameter.percent))); % Select Top K% of features
end
...
if NBSPredict.parameter.ifSave
    ...
    fileDir = [saveDir, 'NBSPredict_Top5.mat'];
    if ~isfolder(saveDir)
