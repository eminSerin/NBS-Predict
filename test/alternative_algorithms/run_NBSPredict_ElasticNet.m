...
%   - It only uses the elastic net algorithm and does not perform any multi
%       model optimization.
%   - Features (edges) are selected through the combination of L1 and L2
%       penalization. 
...
%       96-110.
%   Glmnet for Matlab (2013) Qian, J., Hastie, T., Friedman, J.,
%       Tibshirani, R. and Simon, N.
...
% Codes specific for ElasticNet
% If desired, performs hyperparameter optimziation using random search with
% iteration numbers of hyperOptSteps*2.
NBSPredict.parameter.ifModelOpt = 0;
if NBSPredict.parameter.ifHyperOpt
    hyperOptSteps = 10; % hyperparameters
    nIter = 10; % random search iterations.
    paramGrids.lambda = logspace(-2,3,hyperOptSteps);
    paramGrids.alpha = linspace(1e-64,0.9,hyperOptSteps);
    NBSPredict.parameter.paramGrids = paramGrids;
    NBSPredict.parameter.nIter = nIter;
    featSelParams = {'kFold',NBSPredict.parameter.kFold,...
        'nIter',NBSPredict.parameter.nIter,...
        'bestParamMethod',NBSPredict.parameter.bestParamMethod,...
...
if NBSPredict.parameter.ifClass
    ...
    NBSPredict.parameter.MLmodels = {'ElasticNetR'};
end
