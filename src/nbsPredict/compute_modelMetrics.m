...
nClass = numel(unique([y_true,y_pred])); % Number of unique classes.
...
elseif ~ismember(metrics,{'mae','rmse','r-squared','explained_variance'})
    % If classification. 
    ...
    for i=1:nClass
        for j=1:nClass
            cMatrix(i, j) = sum((y_true == i) & (y_pred == j));
        end
    end
    if nClass <= 2
